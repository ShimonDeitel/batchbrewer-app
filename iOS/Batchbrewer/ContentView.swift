...
struct ContentView: View {
    ...
    @State private var editingEntry: BatchEntry?
        ...
        NavigationStack {
                ...
                ForEach(store.entries) { entry in
                    Button(action: { editingEntry = entry }) {
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            Text(entry.startDate, style: .date).font(.caption).foregroundStyle(.secondary)
                        }
                        .padding(.vertical, 4)
                    }
            ...
            .overlay {
                if store.entries.isEmpty {
                    ContentUnavailableView("No Batches Yet", systemImage: "tray", description: Text("Tap + to add your first batch."))
                }
            }
        }
        .tint(Theme.accent)
...
struct EntryFormView: View {
    ...
    let existing: BatchEntry?
    let onSave: (BatchEntry) -> Void
    ...
    init(entry: BatchEntry?, onSave: @escaping (BatchEntry) -> Void) {
        self.existing = entry
        self.onSave = onSave
        _name = State(initialValue: entry?.name ?? "")
        _startDate = State(initialValue: entry?.startDate ?? Date())
        _recipeNotes = State(initialValue: entry?.recipeNotes ?? "")
        ...
        NavigationStack {
            Form {
                ...
                TextField("RecipeNotes", text: $recipeNotes)
                    .focused($isFocused)
                    .accessibilityIdentifier("form_recipeNotesField")
                TextField("Status", text: $status)
                    .focused($isFocused)
                    .accessibilityIdentifier("form_statusField")
            }
    ...
    private func save() {
        ...
        let entry = BatchEntry(id: id, name: name, startDate: startDate, recipeNotes: recipeNotes, status: status)
        onSave(entry)
    }
}
